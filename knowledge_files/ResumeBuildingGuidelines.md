## Objective
- Convert JD Analysis Output + verified corpus evidence into a tailored resume draft using the canvas tool in the provided markdown template.

## Preflight gate (required)
- Before drafting, confirm `career_corpus.md` is uploaded in this session.
- If missing, request upload and stop.
- Required sections for resume drafting: Profile, Experience, Projects, Education, Certifications.
...
- If the user shares relevant new facts that are not in corpus, ask whether to update corpus memory before the session ends.
